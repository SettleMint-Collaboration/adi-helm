...
# ADI Stack Support Infrastructure Uninstaller
...
Cleanly removes Contour (ingress) and cert-manager (TLS) from your cluster,
including handling of stuck CRDs and resources with finalizers.

Usage: $0 [options]

Options:
  -f, --force             Force removal of stuck resources and CRDs
  -c, --contour-only      Only uninstall Contour (keep cert-manager)
  -m, --certmanager-only  Only uninstall cert-manager (keep Contour)
  -d, --dry-run           Show what would be uninstalled without uninstalling
  -h, --help              Show this help message

Examples:
  $0                      # Uninstall both Contour and cert-manager
  $0 -f                   # Force uninstall with stuck resource cleanup
  $0 -c                   # Only uninstall Contour
  $0 -m -f                # Force uninstall only cert-manager

Warning:
  This will remove all ingress routing and TLS certificates from your cluster.
  Make sure no adi-stack deployments are using these services before uninstalling.
...
# Validate options
if [[ "$CONTOUR_ONLY" == true && "$CERTMANAGER_ONLY" == true ]]; then
    log_error "Cannot use both -c/--contour-only and -m/--certmanager-only"
fi
...
echo ""
echo "=============================================="
echo "  ADI Stack Support Infrastructure Uninstaller"
echo "=============================================="
echo ""
...
# Check for active adi-stack deployments
if [[ "$DRY_RUN" == false ]]; then
    ADI_RELEASES=$(helm list -A --filter 'adi-stack' -o json 2>/dev/null | jq -r '.[].name' 2>/dev/null || true)
    if [[ -n "$ADI_RELEASES" ]]; then
        log_warn "Active adi-stack deployments found:"
        echo "$ADI_RELEASES" | while read -r rel; do
            echo "  - $rel"
        done
        echo ""
        log_warn "These deployments will lose ingress routing and TLS certificates."
        echo ""
...
if [[ "$CERTMANAGER_ONLY" == true ]]; then
    ...
    # Uninstall both - cert-manager first, then Contour
    uninstall_certmanager "$FORCE" "$DRY_RUN"
    echo ""
    uninstall_contour "$FORCE" "$DRY_RUN"
fi
