...
# ADI Stack Uninstaller
# Cleanly removes adi-stack deployments with stuck resource handling
...
Cleanly removes adi-stack Helm releases and their namespaces,
including handling of stuck resources with finalizers.

Usage: $0 [options]

Options:
  -n, --namespace <ns>    Namespace to uninstall (default: adi-stack)
  -r, --release <name>    Helm release name (default: adi-stack)
  -a, --all               Uninstall all adi-stack releases in cluster
...
Examples:
  ...
  $0 -n adi-testnet               # Uninstall from adi-testnet namespace
  $0 -n adi-mainnet -f            # Force uninstall from adi-mainnet
  $0 -a                           # Uninstall all adi-stack releases
...
remove_finalizers() {
    ...
    # Remove finalizers from challenges (cert-manager)
    for challenge in $(kubectl get challenges -n "$namespace" -o name 2>/dev/null || true); do
        log_info "  Removing finalizers from $challenge"
        kubectl patch "$challenge" -n "$namespace" \
            -p '{"metadata":{"finalizers":null}}' --type=merge 2>/dev/null || true
    done

    # Remove finalizers from certificates
    ...
    # Remove finalizers from certificate requests
    for cr in $(kubectl get certificaterequests -n "$namespace" -o name 2>/dev/null || true); do
        log_info "  Removing finalizers from $cr"
        kubectl patch "$cr" -n "$namespace" \
            -p '{"metadata":{"finalizers":null}}' --type=merge 2>/dev/null || true
    done

    # Remove finalizers from orders
    for order in $(kubectl get orders -n "$namespace" -o name 2>/dev/null || true); do
        log_info "  Removing finalizers from $order"
        kubectl patch "$order" -n "$namespace" \
            -p '{"metadata":{"finalizers":null}}' --type=merge 2>/dev/null || true
    done
...
echo ""
echo "=============================================="
echo "  ADI Stack Uninstaller"
echo "=============================================="
echo ""

if [[ "$UNINSTALL_ALL" == true ]]; then
    # Find all adi-stack releases
    log_info "Finding all adi-stack releases..."
    RELEASES=$(helm list -A --filter 'adi-stack' -o json | jq -r '.[] | "\(.namespace) \(.name)"' 2>/dev/null || true)

    if [[ -z "$RELEASES" ]]; then
        log_info "No adi-stack releases found"
        exit 0
    fi

    echo "$RELEASES" | while read -r ns rel; do
        if [[ -n "$ns" && -n "$rel" ]]; then
            uninstall_release "$ns" "$rel" "$FORCE" "$DRY_RUN"
            echo ""
        fi
    done
else
    uninstall_release "$NAMESPACE" "$RELEASE_NAME" "$FORCE" "$DRY_RUN"
fi
